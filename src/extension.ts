...
// ─── 실행할 명령어 정의 ───────────────────────────────────────────
interface KiroCommand {
  id: string;
  label: string;
  command: string;
  icon: string;
  group: 'core' | 'mcp' | 'workflow' | 'etc' | 'custom';
  reuseTerminal?: boolean;
}

// 터미널 컨텍스트 메뉴 커맨드 (현재 활성 터미널에 /agent swap 전송)
const TERMINAL_MENU_COMMANDS: Array<{ id: string; agentName: string }> = [
...
// ─── Terminal 헬퍼 ────────────────────────────────────────────────
function runInTerminal(command: string, terminalName = 'Kiro', reuseTerminal = true) {
...
async function showQuickPick() {
  ...
  const customCmds: { label: string; command: string }[] = config.get('customCommands') ?? [];
...
// ─── Webview 사이드바 패널 ────────────────────────────────────────
class KiroViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'kiroPanel';
  ...
  private async _promptCustomCommand() {
    const input = await vscode.window.showInputBox({
      prompt: '실행할 kiro 명령어를 입력하세요',
      placeHolder: 'kiro-cli ...',
    });
    if (input) runInTerminal(input);
  }
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(KiroViewProvider.viewType, provider)
  );
  ...
  // 커맨드 팔레트
  context.subscriptions.push(
    vscode.commands.registerCommand('kiro.quickPick', showQuickPick)
  );

  // 개별 커맨드 등록
  BUILTIN_COMMANDS.forEach(c => {
    context.subscriptions.push(
      vscode.commands.registerCommand(`kiro.${c.id}`, () =>
        runInTerminal(c.command, `Kiro: ${c.label}`, c.reuseTerminal ?? false)
      )
    );
  });

  context.subscriptions.push(
    vscode.commands.registerCommand('kiro.runCustom', async () => {
      const input = await vscode.window.showInputBox({
        prompt: '실행할 kiro 명령어를 입력하세요',
        placeHolder: 'kiro-cli ...',
      });
